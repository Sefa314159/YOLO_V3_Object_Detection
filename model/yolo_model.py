...
class YOLO:
    ...
    def __init__(self, obj_threshold, nms_threshold):
        ...
        self.yolo             = load_model('data/yolo.h5')
    # -----------------------------------------------------------------------------------
    def process_feats(self, out, anchors, mask):
    ...
    def yolo_out(self, outs, shape):
        ...
        for out, mask in zip(outs, masks):
            b, c, s = self.process_feats(out, anchors, mask)
    ...
    def predict(self, image, shape):
        ...
        outs                   = self.yolo.predict(image)
        boxes, classes, scores = self.yolo_out(outs, shape)
        return boxes, classes, scores
    # -----------------------------------------------------------------------------------